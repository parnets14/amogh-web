...
export default function ProductManagement() {
  ...
  useEffect(() => {
    fetchCategories();
    // fetchProducts depends on currentPage, so called via useEffect below
  }, []);
  ...
  // Create a category ID to name lookup map:
  ...
  const openEditModal = (product) => {
    ...
    setValue("name", product.name);
    setValue("brand", product.brand);
    setValue("category", product.category?._id || (typeof product.category === "string" ? product.category : ""));
    setValue("model", product.model);
    setValue("sku", product.sku);
    setValue("price", product.price);
    setValue("discount", product.discount || 0);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      formData.append(
        "specifications",
        JSON.stringify(data.specifications?.split("\n").filter(Boolean) || [])
      );
      formData.append("usage", data.usage);
      formData.append("status", data.status || "Active");
  ...
  const filteredProducts = products.filter((product) => {
    ...
    // Handle product.category as either string id or object
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Basic Information */}
                  <div className="space-y-4">
                    <h3 className="font-medium text-lg text-gray-700 border-b pb-2">
                      Basic Information
                    </h3>
                            ...
                            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 flex items-center justify-center space-x-2 opacity-0 group-hover:opacity-100 transition-all duration-200 rounded-lg">
                              <button
                                ...
                                className={`p-2 rounded-full ${
                                  image.isPrimary ? "bg-blue-500 text-white" : "bg-white text-gray-800"
                                }`}
                                title={image.isPrimary ? "Primary image" : "Set as primary"}
                              >
                                {image.isPrimary ? "✓" : "★"}
                ...
                <div className="mt-6 flex justify-end space-x-3">
                  ...
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className={`px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition ${
                      isSubmitting
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredProducts.length > 0 ? (
                filteredProducts.map((product) => {
                  const status = getStatus(product.stock);
                  const discountedPrice = product.price * (1 - product.discount / 100);
                  // Resolve category name using categoryMap or product.category.name
                  const productCategoryId =
                    typeof product.category === "string" ? product.category : product.category?._id;
                  const categoryName =
                    product.category?.name || categoryMap[productCategoryId] || "Uncategorized";
                  ...
                  return (
                    <tr key={product._id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-4 text-sm text-gray-500">{product.stock}</td>
                      <td className="px-4 py-4 text-sm text-gray-500">
                        <div className="flex flex-col">
                          <span
                            className={
                              product.discount > 0 ? "line-through text-gray-400 text-xs" : "text-sm"
                            }
                          >
                            ₹{product.price.toFixed(2)}
                          </span>
