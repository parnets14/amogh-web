...
export default function ProductDetail() {
  ...
  const isInCart = () => {
    return cartItems.some((item) => item.instrumentId === product._id)
  }
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto py-12 px-4 text-center">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#01A4D5] mx-auto mb-4"></div>
        <p>Loading product details...</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="max-w-7xl mx-auto py-12 px-4 text-center">
        ...
        <button onClick={() => navigate(-1)} className="px-4 py-2 bg-[#01A4D5] text-white rounded-lg">
          Go Back
        </button>
      </div>
    )
      ...
      <nav className="flex mb-6" aria-label="Breadcrumb">
        <ol className="inline-flex items-center space-x-1 md:space-x-3">
          <li>
            <div className="flex items-center">
              <ChevronLeft className="w-4 h-4 mr-1" />
              <Link to="/allproduct" className="text-sm font-medium text-gray-700 hover:text-[#01A4D5]">
                All Products
              </Link>
            </div>
          </li>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 rounded-xl p-6 flex items-center justify-center h-96">
            {product.images?.length > 0 ? (
              <img
                src={getImageUrl(product.images[selectedImageIndex]?.url) || "/placeholder.svg"}
                alt={product.name}
                className="h-full w-full object-contain"
                loading="lazy"
                onError={(e) => {
        ...
        <div>
          {/* Brand & Title */}
          <div className="mb-4">
            {product.brand && <span className="text-sm text-gray-500">Brand: {product.brand}</span>}
            <h1 className="text-3xl font-bold mt-1">{product.name}</h1>
            {product.model && <span className="text-sm text-gray-500">Model: {product.model}</span>}
            {product.sku && <span className="text-sm text-gray-500 block">SKU: {product.sku}</span>}
            <div className="flex items-center mt-2">
              <div className="flex">
                {[...Array(5)].map((_, i) => (
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-500">Price</p>
                <div className="flex items-center">
                  <p className="text-3xl font-bold text-[#01A4D5]">${product.price?.toLocaleString() ?? "0"}</p>
                  {product.discount > 0 && (
                    <>
                      <span className="ml-2 text-sm line-through text-gray-500">
                        $
                        {(product.price / (1 - product.discount / 100)).toLocaleString(undefined, {
          ...
          <div className="mb-8">
            <div className="flex items-center mb-4">
              ...
              {product.stock > 0 && <span className="ml-2 text-sm text-gray-500">{product.stock} available</span>}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                {isInCart() ? (
                  <>
                    <Check className="w-5 h-5 mr-2" /> In Cart
                  </>
                ) : product.stock === 0 ? (
                  "Out of Stock"
                ) : (
            ...
            <div className="mt-3">
              ...
              >
                <Heart className={`w-5 h-5 mr-2 ${inWishlist ? "fill-red-600 text-red-600" : "text-gray-500"}`} />
                {inWishlist ? "Saved" : "Save for Later"}
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 text-center mb-8">
            ...
            <div className="p-3 bg-gray-50 rounded-lg">
              <Shield className="w-6 h-6 mx-auto text-[#01A4D5] mb-1" />
              <p className="text-xs text-gray-600">{product.warranty || "No warranty"}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-lg">
              <CreditCard className="w-6 h-6 mx-auto text-[#01A4D5] mb-1" />
              <p className="text-xs text-gray-600">Secure payment</p>
